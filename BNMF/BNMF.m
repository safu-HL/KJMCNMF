function [W,H1,H2,H3,H4] = BNMF(X1, X2, X3, X4,L1,L2,L3,L4, K, maxiter, speak, fid)
%
...
%     Lh1=std2(H1);
    ...
    Lh3=std2(H3);
    Lh4=std2(H4);
    W = W.*([H2 H3 H4]*[X2 X3 X4]')'./(W*([H2 H3 H4]*[H2 H3 H4]'+((L2/Lw)+(L3/Lw)+(L4/Lw))*eye(K))+eps);      
%     HH1 = H1.*(W'*X1)./((W'*W + (L1/Lh1)*eye(K))*H1 +eps);     
    HH2 = H2.*(W'*X2)./((W'*W + (L2/Lh2)*eye(K))*H2 +eps);
    HH3 = H3.*(W'*X3)./((W'*W + (L3/Lh3)*eye(K))*H3 +eps);
    HH4 = H4.*(W'*X4)./((W'*W + (L4/Lh4)*eye(K))*H4 +eps);
